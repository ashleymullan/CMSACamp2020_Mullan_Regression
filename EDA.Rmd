...
data_avg_only <- with_averages_data %>%
  select(avg_points, team, season, avg_turnovers, avg_fouls, avg_assists, avg_rebounds, avg_score_differential, avg_steals, avg_blocks, avg_blocks_against, win_perc, field_goal_perc, three_point_perc, free_throw_perc, two_point_perc)
head(data_avg_only)
...
fit_lasso_points_cv <- cv.glmnet(points_model_x, points_model_y, foldid = data_copy_for_adding_fold_id$fold, alpha = 1)
...
fit_lasso_differential_cv <- cv.glmnet(differential_model_x, differential_model_y, foldid = data_copy_for_adding_fold_id$fold, alpha = 1)
